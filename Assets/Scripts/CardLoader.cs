...
{
    private Dictionary<string, (List<Sprite>,List<string>)> cardDatabase;
    ...
    */
    ...
    IEnumerator Start()
    {
        cardDatabase = new Dictionary<string, (List<Sprite>,List<string>)>();
        yield return StartCoroutine(InitializeExtractPath());
    ...
    void LoadCards()
    {
        extractPath = Path.Combine(extractPath, "Choices"); /**/
        ...
        foreach (string subjectFolder in subjectFolders)
        {
            ...
            foreach (string cardFile in cardFiles)
            {
                ...
                string cardName = cardFile; //THIS TO CHANGE TO NAME
                if (cardSprite != null)
                {
                    currentCards.Add(cardSprite);
                    currentNames.Add(cardName);
                }
            }
            cardDatabase.Add(subjectName, (currentCards,currentNames));
        }
        //
    ...
    */
    ...
    public (List<Sprite>,List<string>) ListAllSprites(string subject)
    {
        List<Sprite> cards = new List<Sprite>();
        List<string> myNames = new List<string>();
        if (cardDatabase.ContainsKey(subject))
        {
            List<Sprite> sprites = cardDatabase[subject].Item1;
            List<string> names = cardDatabase[subject].Item2;
            ...
            {
                //myNames.Add(name);
                int lastSeparatorIndex = Mathf.Max(name.LastIndexOf('/'), name.LastIndexOf('\\'));
                int lastDotIndex = name.LastIndexOf('.');
                myNames.Add(name.Substring(lastSeparatorIndex + 1, lastDotIndex - lastSeparatorIndex - 1));
            }
        }
        else
        {
    ...
    public void ChangeCurrentAmountToPick(int num)
    {
        amountToPick = num;
    }

    public int GetCurrentAmountToPick()
    {
        return amountToPick;
    }
